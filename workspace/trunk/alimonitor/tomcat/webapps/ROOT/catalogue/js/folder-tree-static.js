...
*/
	...
	var ajaxRequestFile = 'getFolderList.jsp';
	...
	*/
	...
	function expandLabels(treeId, labels){
	    var menuItems = document.getElementById(treeId).getElementsByTagName('LI');
	    
	    for (var i=0; i<labels.length; i++){
		for(var no=0;no<menuItems.length;no++){
		    var aList = menuItems[no].getElementsByTagName('A');
		    
		    if (aList.length==0)
			continue;
		
		    var thisPath = aList[0].getAttribute("name");
		
		    if (thisPath == labels[i] || thisPath == "/" || labels[i].indexOf(thisPath+"/")==0){
			//alert('found '+labels[i]+" -- "+thisPath);
	...
	function showHideNode(e,inputId)
	{
		...
		inputId = parentNode.id.replace(/[^0-9]/g,'');
			...
			try{
			    if (!isLoading)
				lf(parentNode.getElementsByTagName('A')[0].getAttribute('path'));
			}
			catch (e){
			}
