	...
	************************************************************************************************************/
	...
	function showContextMenu(e)
	{
		...
		contextMenuObj.fullPath = targ.getAttribute("fullpath");
		
		var f = contextMenuObj.fullPath.toLowerCase();
		f = f.substring(f.lastIndexOf('/')+1);
		
		if (f.match("\\.c.?$") || f.match("jdl.?$") || f.match("\\.sh.?$") || f.match("post_install.?$") || f.match("aliroot_new.?$") || f.match("aliroot.?$") || f.match("\\.txt.?$") || f.match("readme.?") || f.match("\\.rc.?$")){
		    objById('editOption').style.display = 'block';
		}
		else{
		    objById('editOption').style.display = 'none';
		}
		
		if (f.match("jdl.?$")){
		    objById('submitOption').style.display = 'block';
		}
		else{
		    objById('submitOption').style.display = 'none';
		}
