<%
    lia.web.servlets.web.Utils.logRequest("START /console.jsp", 0, request);
...
<script type="text/javascript">
    ...
    function clearWin(){
	tail_frame.document.getElementById('console_output').innerHTML = '';
    }
    
    function highlight(){
	var tailf = tail_frame.document.getElementById('console_output');
	...
	var tailf = window.frames.tail_frame;
	...
	var p = tail_frame.document.getElementById('console_output');
	...
	if (scrollEnabled){
	    tail_frame.window.scrollBy(0, 10000000);
	}
    }
	...
	addText('&gt; <a href="javascript:void(0);" onClick="return parent.prepareCommand(\''+escape(cmd)+'\');" title="'+(new Date())+'">'+cmd+'</a><br>');
    
	new Ajax.Request(
	        '/console_command.jsp?site=<%=site%>&command='+escape(cmd),
	        {
	            method: 'get',
	            asynchronous: false,
	            onSuccess: newText
...
<table style="font-family:Arial,Verdana,Helvetica;font-size:12px" class="text" cellspacing="0" cellpadding="0" width=100% height=100%>
    <tr height=20>
	<td align=center style="font-size:14px" nowrap><b>Console on <%=site%></b></td>
    </tr>
    <tr>
	<td style="border: solid 2px #CCCCCC; padding: 5px;">
	    <iframe id="tail_frame" name="tail_frame" width="100%" height="100%" frameborder="0" src="console_iframe.html"></iframe>
	</td>
    </tr>
    <tr height="40">
	<td style="border: solid 1px #CCCCCC; border-top: 0px">
	...
	<td colspan=4 style="padding-left: 10px; padding-right: 10px;color: #67674D; font-weight: bold">
	    Command:<br>
	    <form name="command_executor" action="console.jsp" onSubmit="return executeCommand();">
...
<script type="text/javascript">
    ...
    lia.web.servlets.web.Utils.logRequest("/console.jsp", 0, request);
%>