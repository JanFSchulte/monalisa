<%@ page import="alimonitor.*,auth.*,lazyj.*,java.io.*,java.util.*,lia.Monitor.Store.Fast.DB,alien.daq.*,java.util.concurrent.atomic.*,utils.IntervalQuery,java.sql.*"%><%!
	...
	private void fetchDetails() throws SQLException {
		...
		for (int i=1; i<=db.getMetaData().getColumnCount(); i++){
	        ...
	        if (events<0){
	    	    db2.query("SELECT * FROM job_events WHERE pid="+pid);
	    	    
	    	    if (db2.moveNext()){
	    		if (db2.geti("train_passed")>0)
	    		    events = db2.geti("train_passed");
	    	    }
	        }
			...
			for (int i=1; i<=db.getMetaData().getColumnCount(); i++){
	    	    ...
	    	    if (!sComment.equals(sType) && !sComment.equals(sIssues)){
...
%><%
    ...
    String sBookmark = "/runview/?run="+iRun;
    ...
    db.query("select masterjob_id from (SELECT masterjob_id FROM rawdata_processing_requests where run="+iRun+" union select pid from lpm_history where runno="+iRun+" and parentpid=0 and jdl like '%/rec_pass%.jdl' order by masterjob_id asc) x inner join job_runs_details on pid=masterjob_id;;");
