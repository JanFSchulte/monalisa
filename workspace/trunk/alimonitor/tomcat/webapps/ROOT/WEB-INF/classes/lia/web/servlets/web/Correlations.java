...
public class Correlations extends ThreadedPage {
	...
	public void execGet(){
		...
		final HashSet<String> hsExcludedTags = new HashSet<String>(EXCLUDED_TAGS);
		...
		while (e.hasMoreElements()){
			...
			if (values.length>1){
				final StringBuilder sb = new StringBuilder(values[0]);
	...
	private Plot getPlot(final Properties prop, final String sChartName) {
		...
		if (pgetb(prop, sChartName+".last_values", pgetb(prop, "last_values", false))){
		    ds = new DataSplitter();
		    
		    for (monPredicate p: uniquePreds){
			final Vector<Object> vo = Cache.getLastValues(p);
		    ...
		    final boolean bNormalQuery = pgetb(prop, sChartName+".normal_query", pgetb(prop,"normal_query", sType.equals("history")));
		    
		    if (bNormalQuery)
		    	ds = store.getDataSplitter(uniquePreds.toArray(new monPredicate[0]), lCompactInterval);
		    else
			ds = store.getUniformDataSplitter(uniquePreds.toArray(new monPredicate[0]), lCompactInterval);
		}
		...
		for (int i=0; i<vsNames.length; i++){
			...
			for (monPredicate pred: preds){
				...
				final Vector<Object> v = ds.getAndFilter(pred);
