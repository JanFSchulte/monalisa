...
public class SampleServlet extends HttpServlet {
	...
	private transient TransparentStoreFast store;
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		monPredicate p = new monPredicate("GLORIAD", // Farm
			"WAN", // Cluster
			"*", // Node (any)
			-30 * 60 * 1000, -1, // start time, end time, in millis, if
			// negative then they are relative to
			// the current time
			new String[] { "Russia_IN" }, // list of functions, leave this
			// parameter null for every
			// function
			null);
		...
		if (r != null) {
			pw.println((new Date(r.time)).toString() + " : " + r.FarmName + " / " + r.ClusterName + " / " + r.NodeName
				+ " / " + r.param_name[0] + "=" + r.param[0] + "<br>");
		}

		pw.println("<br><br>History info:<br><br>");
		// Retrieve the history information from the database
		Vector<Object> vData = store.getResults(new monPredicate[] { p });
		for (int i = 0; i < vData.size(); i++) {
			ExtendedResult er = (ExtendedResult) vData.get(i);

			pw.println((new Date(er.time)).toString() + " : " + er.FarmName + " / " + er.ClusterName + " / "
				+ er.NodeName + " / " + er.param_name[0] + "=" + er.param[0] + " (min:" + er.min + ", max:" + er.max
				+ ")" + "<br>");
		}
