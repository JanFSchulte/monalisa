<%@ page import="alimonitor.*,lia.web.utils.ThreadedPage,lia.Monitor.Store.Fast.DB,lia.Monitor.Store.Cache,lia.Monitor.Store.FarmBan,lia.web.utils.Formatare,lia.Monitor.monitor.monPredicate,java.io.*,java.util.*,java.net.URLEncoder,lia.web.servlets.web.*,lia.Monitor.monitor.Result,lia.Monitor.monitor.eResult,lia.Monitor.JiniClient.Store.Main" %><%
    lia.web.servlets.web.Utils.logRequest("START /centralservices/index.jsp", 0, request);
    ...
    pMaster.modify("title", "Central services admin");
    
    //menu
    pMaster.modify("class_siteservices", "_active");

    final Page p = new Page("centralservices/index.res");
    
    final Page pService = new Page("centralservices/index_line.res");
    
    final Page pStatus = new Page("centralservices/index_detail.res");
    
    final DB db = new DB("SELECT name,ip FROM abping_aliases WHERE name like 'aliendb%.cern.ch' or name='CERN' order by 1;");
    ...
    final String[] vsServices = new String[]{"Authen", "PackMan", "JobBroker", "FTD", "IS"};
