...
// simrun.C
{
  ...
  gSystem->Exec("mkdir gensim");
  gSystem->Exec("mv galice.root galice_sim.root");
  gSystem->Exec("mv *.root gensim");
  gSystem->Exec("ln -s gensim/geometry.root");
  gSystem->Exec("ln -s gensim/raw.root");
  gSystem->Exec("aliroot -b -q rec.C 2>&1 | tee rec.log");
  gSystem->Exec("cp galice.root galice_rec.root");
  gSystem->Exec("mv gensim/*.root .");
