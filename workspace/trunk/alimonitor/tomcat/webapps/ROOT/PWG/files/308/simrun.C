...
//
// Reconstruction with SDD
  cout<<">>>>> RECONSTRUCTION with SDD <<<<<"<<endl;
...
//
// Cleanup
  gSystem->Exec("mkdir wSDD");	
  gSystem->Exec("mv AliESDs.root AliESDs_wSDD.root");	
  gSystem->Exec("mv AliESDfriends.root AliESDfriends_wSDD.root");
  gSystem->Exec("rm *.RecPoints.root");
  cout<<">>>>> RECONSTRUCTION without SDD <<<<<"<<endl;
  gSystem->Exec("aliroot -b -q recNoSDD.C >> rec.log 2>&1");
  gSystem->Exec("cat syswatch.log >> recwatch.log");
  cout<<">>>>> TAG (2) <<<<<"<<endl;
  gSystem->Exec("aliroot -b -q tag.C >> tag.log 2>&1");
  cout<<">>>>> CHECK ESD (2) <<<<<"<<endl;
  gSystem->Exec("aliroot -b -q CheckESD.C >> check.log 2>&1");
}
