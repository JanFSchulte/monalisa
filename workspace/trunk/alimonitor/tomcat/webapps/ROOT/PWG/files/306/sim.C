  ...
  if (!(strcmp(embrun,"kBackground"))){
    ...
    simulator.SetMakeSDigits("MUON ITS");
		...
		if (trgname){
			...
			TString strgname(trgname);
			TObjArray *tClasses = strgname.Tokenize(":");
      Bool_t tTrig = kFALSE;
			Int_t triggerval = 0;
