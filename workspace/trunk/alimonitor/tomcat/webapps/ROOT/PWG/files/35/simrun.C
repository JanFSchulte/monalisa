...
// root.exe -b -q simrun.C  --run <x> --event <y> --field <kNoField/k5kG>
...
// Needed to produce simulated RAW data
  ...
  gSystem->Exec("(pwd; ls -l; aliroot -b -q sim.C) > sim.log 2>&1");
  cout<<">>>>> RECONSTRUCTION <<<<<"<<endl;
  gSystem->Exec("(pwd; ls -l; aliroot -b -q rec.C) > rec.log 2>&1");
  cout<<">>>>> TAG <<<<<"<<endl;
  gSystem->Exec("(pwd; ls -l; aliroot -b -q tag.C) > tag.log 2>&1");
  cout<<">>>>> CHECK ESD <<<<<"<<endl;
  gSystem->Exec("(pwd; ls -l; aliroot -b -q CheckESD.C) > check.log 2>&1");
  cout<<">>>>> AOD <<<<<"<<endl;
  gSystem->Exec("(pwd; ls -l; aliroot -b -q CreateAODfromESD.C) > aod.log 2>&1");
}
