//
// Configuration file for displaced vertices (C.Cheshkov and M.Guilbaud)
//
//
...
static PDC06Proc_t   proc     = kHijing2000;
static Mag_t         mag      = k5kG;
static Float_t       energy   = 2760.; // energy in CMS
static Float_t       bMin     = 6.;
static Float_t       bMax =   = 12.;
...
void Config()
{
  ...
  Int_t ivertex = gRandom->Integer(16);
  Float_t zvertex = -187.5+(Float_t)ivertex*37.5;
  Float_t time0 = (Float_t)ivertex*1.25e-9-6.25e-9;    
  ...
  if (vertex) {
      ...
      cout << "Overwriting Config.C vertex settings !" << endl;
      cout << Form("Vertex position from OCDB entry: x = %13.3f, y = %13.3f, z = %13.3f (sigma = %13.3f)\n", vtxPos[0], vtxPos[1], vtxPos[2]+zvertex, vtxSig[2]) << endl;
    
//      Aligenererator *gener = gAlice->GetMCApp()->genererator();
      gener->SetOrigin(vtxPos[0], vtxPos[1], vtxPos[2]+zvertex);   // vertex position
      gener->SetSigmaZ(vtxSig[2]);
      }
  }
  
  cout << Form("Generated satellite index = %d with time0 = %.5f\n",ivertex,time0) << endl;
  gener->SetTimeOrigin(time0);
...
AliGenerator* Hijing2000()
{
    ...
    gener->SetImpactParameterRange(6., 12.);	
    gener->SetJetQuenching(0);	
