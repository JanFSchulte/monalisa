...
//		}
		...
		// loop over the calorimeters clusters
		{
			...
			for (Int_t iCluster=0; iCluster<nCaloCluster; iCluster++){
				//cout<<"i "<<iCluster<<endl;
				//cout<<"E "<<esd->GetCaloCluster(iCluster)->E()<<"; PHOS "<<esd->GetCaloCluster(iCluster)->IsPHOS()<<"; EMCAL "<<esd->GetCaloCluster(iCluster)->IsEMCAL()<<endl;     
				if(esd->GetCaloCluster(iCluster)->IsPHOS()) 
					hEPHOS->Fill(esd->GetCaloCluster(iCluster)->E());
				else if(esd->GetCaloCluster(iCluster)->IsEMCAL()) {
					hEEMCAL->Fill(esd->GetCaloCluster(iCluster)->E());
				}
			}
		}
	}
	cout<<"Perform checks"<<endl;
	// perform checks
