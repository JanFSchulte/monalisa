...
enum PDC06Proc_t 
{
  kPythia6, kPythia6D6T, kPythia6ATLAS, kPythia6ATLAS_Flat, kPythiaPerugia0, kPhojet, kHijing, kHijing2000, kHydjet, kDpmjet, kRunMax
};

const char * pprRunName[] = {
  "kPythia6", "kPythia6D6T", "kPythia6ATLAS", "kPythia6ATLAS_Flat", "kPythiaPerugia0", "kPhojet", "kHijing", "kHijing2000", "kHydjet", "kDpmjet"
};
...
void Config()
{
  ...
  if (proc == kHijing || proc == kHijing2000) { 	
...
// neutral pion and heavy particle decays switched off
     gener->SetDecaysOff(3); // 3 requested by Ana Marin
...
AliGenerator* Hijing2000()
{
  ...
  Float_t thmin = EtaToTheta(0.8);   // theta min. <---> eta max
  Float_t thmax = EtaToTheta(-0.8);  // theta max. <---> eta min 
  ...
  box2->SetMomentumRange(0.2, 3.0);
  box2->SetPhiRange(0., 360.);
  box2->SetThetaRange(thmin,thmax); 

  // 3. Anti-Deuteron
  AliGenBox *box3 = new AliGenBox(1);
  box3->SetPart(-1000010020);   // for anti- deu
  box3->SetMomentumRange(0.2, 3.0);
  box3->SetPhiRange(0., 360.);
  box3->SetThetaRange(thmin,thmax); 

  // 4. Triton
  AliGenBox *box4 = new AliGenBox(1);
  box4->SetPart(1000010030);   // for tri
  box4->SetMomentumRange(0.2, 3.0);
  box4->SetPhiRange(0., 360.);
  box4->SetThetaRange(thmin,thmax); 

  // 5. Anti-Triton
  AliGenBox *box5 = new AliGenBox(1);
  box5->SetPart(-1000010030);   // for anti-tri
  box5->SetMomentumRange(0.2, 3.0);
  box5->SetPhiRange(0., 360.);
  box5->SetThetaRange(thmin,thmax); 

  // 6. Helium3
  AliGenBox *box6 = new AliGenBox(1);
  box6->SetPart(1000020030);   // for Helium3
  box6->SetMomentumRange(0.3, 6.0);
  box6->SetPhiRange(0., 360.);
  box6->SetThetaRange(thmin,thmax); 

  // 7. Anti-Helium3
  AliGenBox *box7 = new AliGenBox(1);
  box7->SetPart(-1000020030);   // for anti-Helium3
  box7->SetMomentumRange(0.3, 6.0);
  box7->SetPhiRange(0., 360.);
  box7->SetThetaRange(thmin,thmax); 

  // 8. Helium4
  AliGenBox *box8 = new AliGenBox(1);
  box8->SetPart(1000020040);   // for Helium4
  box8->SetMomentumRange(0.5, 6.0);
  box8->SetPhiRange(0., 360.);
  box8->SetThetaRange(thmin,thmax); 

  // 9. Anti-Helium4
  AliGenBox *box9 = new AliGenBox(1);
  box9->SetPart(-1000020040);   // for anti-Helium4
  box9->SetMomentumRange(0.5, 6.0);
  box9->SetPhiRange(0., 360.);
  box9->SetThetaRange(thmin,thmax); 
  ...
  gener->AddGenerator(box9,"fbox9",1);
    ...
    /*
      gh->SetJetQuenching(0);	//doubt
      gh->SetPtHardMin (2.3);	//doubt

    Int_t prt=0;  	//doubt
    AliGenSTRANGElib *lib = new AliGenSTRANGElib();  	//doubt

    prt=AliGenSTRANGElib::kLambda;  	//doubt
    AliGenParam *gl = new AliGenParam(2*4, prt,  // 5 Lambda and 5 anti-Lambda
    ...
    //gom->SetForceDecay(kNoDecay);

    gener->AddGenerator(gh,"hijing",1);
    gener->AddGenerator(gl,"mt-scaled lambda",1);
    gener->AddGenerator(gk0,"flat pt k0s",1);
    gener->AddGenerator(gf,"flat pt lambda",1);
    gener->AddGenerator(gx,"flat pt xi-",1);
    gener->AddGenerator(gx0,"flat pt xi0",1);
    gener->AddGenerator(gom,"flat pt omega-",1);
    */
