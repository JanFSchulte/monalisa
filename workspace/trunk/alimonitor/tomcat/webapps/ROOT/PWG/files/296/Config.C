...
enum PDC06Proc_t 
{
  kPythia6, kPythia6D6T, kPythia6ATLAS, kPythia6ATLAS_Flat, kPythiaPerugia0, kPhojet, kPythiaPerugia0chadr, kPythiaPerugia0bchadr, kPythiaPerugia0cele, kPythiaPerugia0bele, kPythiaPerugia0Jpsi2e, kPythiaPerugia0BtoJpsi2e, kHijing, kHijing2000, kHijing2000HF, kHydjet, kDpmjet, kAmptHF, kAmpt, kPythiaPerugia0HFbarrel, kRunMax
};

const char * pprRunName[] = {
  "kPythia6", "kPythia6D6T", "kPythia6ATLAS", "kPythia6ATLAS_Flat", "kPythiaPerugia0", "kPhojet",  "kPythiaPerugia0chadr", "kPythiaPerugia0bchadr", "kPythiaPerugia0cele", "kPythiaPerugia0bele", "kPythiaPerugia0Jpsi2e", "kPythiaPerugia0BtoJpsi2e", "kHijing", "kHijing2000", "kHijing2000HF", "kHydjet", "kDpmjet", "kAmptHF", "kAmpt", "kPythiaPerugia0HFbarrel"
};
...
static PDC06Proc_t   proc     = kPythiaPerugia0HFbarrel;
...
void Config()
{
  ...
  // --- All positions are in cm, angles in degrees, and P and E in GeV
    ...
    // RANDOM SELECTION OF ONE OF THE TWO GENERATION TYPES
    //
    Int_t typeHF = -1;
    Float_t randHF = gRandom->Rndm();
    
    cerr << "..... randHF = " << randHF << endl;
    if ( randHF <0.5) {
      typeHF=2;
    } else {
      typeHF=3;
  ...
  if (proc == kPythia6) {
      ...
      gener = MbPhojet();
	     ...
	     (proc == kPythiaPerugia0HFbarrel && typeHF==0)) {
      gener = MbPythiaTunePerugia0chadr();
  } else if (proc == kPythiaPerugia0bchadr ||
	     (proc == kPythiaPerugia0HFbarrel && typeHF==1)) {
      gener = MbPythiaTunePerugia0bchadr();
  } else if (proc == kPythiaPerugia0cele ||
	     (proc == kPythiaPerugia0HFbarrel && typeHF==2)) {
      gener = MbPythiaTunePerugia0cele();
  } else if (proc == kPythiaPerugia0bele ||
	     (proc == kPythiaPerugia0HFbarrel && typeHF==3)) {
      gener = MbPythiaTunePerugia0bele();
  } else if (proc == kPythiaPerugia0Jpsi2e ||
	     (proc == kPythiaPerugia0HFbarrel && typeHF==4)) {
      gener = MbPythiaTunePerugia0Jpsi2e();
  } else if (proc == kPythiaPerugia0BtoJpsi2e ||
	     (proc == kPythiaPerugia0HFbarrel && typeHF==5)) {
      gener = MbPythiaTunePerugia0BtoJpsi2e();
  }
...
{
      comment = comment.Append(" pp: Pythia (Perugia0) cele (1 ccbar per event, 1 electron in |y|<0.9");
//
...
{
      comment = comment.Append(" pp: Pythia (Perugia0) bele (1 bbbar per event, 1 electron in |y|<0.9");
//
...
AliGenerator* Hijing2000HF(Int_t typeHF)
{
  ...
  gk0->SetPart(310);
  gk0->SetPtRange(0, 10);
  gk0->SetThetaRange(45,135);

  AliGenBox *gf = new AliGenBox(1);        // 1 Lambda, flat pt 

  gf->SetPart(3122);
  gf->SetPtRange(0, 10);
  gf->SetThetaRange(45,135);

  AliGenBox *gx = new AliGenBox(3);        // 3 Xi-, flat pt 

  gx->SetPart(3312);
  gx->SetPtRange(0, 13);
  gx->SetThetaRange(45,135);

  AliGenBox *gx0 = new AliGenBox(3);       // 3 Xi0, flat pt 

  gx0->SetPart(3322);
  gx0->SetPtRange(0, 13);
  gx0->SetThetaRange(45,135);

  AliGenBox *gom = new AliGenBox(1);       
  Double_t select = gRandom->Rndm();
  if (select < 1./3.) {
      gom->SetPart(3334); // 1 Omega-, flat pt

  }
  else if ( select < 2./3.) {
    gom->SetPart(-3334);// 1 AntiOmega+, flat pt

  }
  else {
    gom->SetPart(-3312);// 1 AntiXi, flat pt

  }
  gom->SetPtRange(0, 13);
  gom->SetThetaRange(45,135);

  cocktail->AddGenerator(gl,  "mt-scaled lambda" ,1);
  cocktail->AddGenerator(gk0, "flat pt k0s"      ,1);
  cocktail->AddGenerator(gf,  "flat pt lambda"   ,1);
  cocktail->AddGenerator(gx,  "flat pt xi-"      ,1);
  cocktail->AddGenerator(gx0, "flat pt xi0"      ,1);
  cocktail->AddGenerator(gom, "flat pt omega-"   ,1);
//
