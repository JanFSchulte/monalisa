//
// Configuration for D2H rare signals pp 7 TeV 2010
// PYTHIA pp 7000 TeV Perugia0
// 40% ccbar pair per event
//     at least one in |y|<1.5
//     D0 mesons decay to hadrons
// 20% ccbar pair per event
//     at least one in |y|<1.5
//     Ds mesons decay to hadrons
// 20% ccbar pair per event
//     at least one in |y|<1.5
//     Lc baryons decay to hadrons
// 20% ccbar pair per event
//     at least one in |y|<1.5
//     D+ mesons decay to hadrons
//
...
enum PDC06Proc_t
{
  kPythia6, kPythia6D6T, kPythia6ATLAS, kPythia6ATLAS_Flat, kPythiaPerugia0, kPhojet, kPythiaPerugia0chadr, kPythiaPerugia0bchadr, kPythiaPerugia0cele, kPythiaPerugia0bele, kPythiaPerugia0Jpsi2e, kPythiaPerugia0BtoJpsi2e, kPythiaPerugia0HFbarrel, kPythiaPerugia0D2H, kRunMax,
};

const char * pprRunName[] = {
  "kPythia6", "kPythia6D6T", "kPythia6ATLAS", "kPythia6ATLAS_Flat", "kPythiaPerugia0", "kPhojet", "kPythiaPerugia0chadr", "kPythiaPerugia0bchadr", "kPythiaPerugia0cele", "kPythiaPerugia0bele", "kPythiaPerugia0Jpsi2e", "kPythiaPerugia0BtoJpsi2e", "kPythiaPerugia0HFbarrel", "kPythiaPerugia0D2H"};
...
static PDC06Proc_t   proc     = kPythiaPerugia0D2H;
...
//    Pythia
      ...
      if(proc==kPythiaPerugia0D2H) {
	Float_t randchadr = gRandom->Rndm();
	if(randchadr<0.4) {
	  pythia->SetTriggerParticle(421); // D0
	  comment.Append(" Trigger: D0");
	} else if (randchadr>=0.4 && randchadr<0.6) {
	  pythia->SetTriggerParticle(411); // D+
	  comment.Append(" Trigger: D+");
	} else if (randchadr>=0.6 && randchadr<0.8) {
	  pythia->SetTriggerParticle(431); // Ds
	  comment.Append(" Trigger: Ds");
	} else {
	  pythia->SetTriggerParticle(4122); // Lc
	  comment.Append(" Trigger: Lc");
	}
      }
      pythia->SetEnergyCMS(energy);
//    Tune
