//
// Configuration for the Lambda(1520) MC production in pp@7TeV based on the Xi or Omega-enhanced MC production 
// (similar to that used in LHC11a6a or LHC11a6b for p+p 7 TeV data) 
//

// MC production corresponding to pp 7 TeV data 
// 50% of events have a generated Lambda(1520)
// 50% of events have a generated Anti-Lambda(1520)
// Introduction of a new AliGenerator = MbPythiaTunePerugia11AndLambda1520Enhancement
//    Pythia MB Perugia11 + Lambda(1520) as Trigger Particle
...
enum PDC06Proc_t 
{
  kPythia6, kPythia6D6T, kPythia6ATLAS, kPythia6ATLAS_Flat, kPythiaPerugia11, kPythiaTunePerugia11AndOmegaEnhancement, kPythiaTunePerugia11AndLambda1520Enhancement, kPythiaTunePerugia11AndXiEnhancement, kPhojet, kRunMax
};

const char * pprRunName[] = {
  "kPythia6", "kPythia6D6T", "kPythia6ATLAS", "kPythia6ATLAS_Flat", "kPythiaPerugia11", "kPythiaTunePerugia11AndOmegaEnhancement", "kPythiaTunePerugia11AndLambda1520Enhancement", "kPythiaTunePerugia11AndXiEnhancement", "kPhojet"
};
...
static PDC06Proc_t   proc     = kPythiaTunePerugia11AndXiEnhancement;
...
//    gSystem->Load("libpythia6.4.21");   // Pythia 6.4
    gSystem->Load("libpythia6.4.25");   // Pythia 6.4.25 needed for tune Perugia-11        
  }
  ...
  } else if (proc == kPythiaTunePerugia11AndLambda1520Enhancement) {
      gener = MbPythiaTunePerugia11AndLambda1520Enhancement();
...
{
      comment = comment.Append(" pp: Pythia low-pt (Perugia11)");
//
//    Pythia
      AliGenPythia* pythia = new AliGenPythia(-1); 
      pythia->SetMomentumRange(0, 999999.);
...
AliGenerator* MbPythiaTunePerugia11AndLambda1520Enhancement()
{
      comment = comment.Append(" pp: Pythia low-pt (Perugia11) + __Lambda1520__ enhancement");
//
//    Pythia
      AliGenPythia* pythia = new AliGenPythia(-1); 
      pythia->SetMomentumRange(0, 999999.);
...
//    350     Perugia 11 
      ...
      pythia->SetCrossingAngle(0,0.000280);
      if (gRandom->Rndm() < 0.5)
	pythia->SetTriggerParticle(3124, 1.2);
      else
	pythia->SetTriggerParticle(-3124, 1.2);
