...
enum PDC06Proc_t 
{
  kPythia6, kPythia6D6T, kPythia6ATLAS, kPythia6ATLAS_Flat, kPythiaPerugia0, kPythiaTunePerugia0NucleiEnhancement, kPhojet, kRunMax
};

const char * pprRunName[] = {
  "kPythia6", "kPythia6D6T", "kPythia6ATLAS", "kPythia6ATLAS_Flat", "kPythiaPerugia0", "kPythiaTunePerugia0NucleiEnhancement","kPhojet" 
};
...
void Config()
{
  ...
  } else if (proc == kPythiaTunePerugia0NucleiEnhancement) {
      gener = MbPythiaTunePerugia0NucleiEnhancement();
...
AliGenerator* MbPythiaTunePerugia0NucleiEnhancement()
{
  comment = comment.Append(" pp: Pythia low-pt + one (Anti)Nuclei ");
  
  // Set pseudorapidity range from -1.0 to 1.0  
  Float_t thmin = EtaToTheta(1.0);   // theta min. <---> eta max  
  Float_t thmax = EtaToTheta(-1.0);  // theta max. <---> eta min 
  
  
  AliGenCocktail *Cocktail = new AliGenCocktail();
  
  // 1. PythiaTunePerugia0
  AliGenPythia* pythia = (AliGenPythia*)  MbPythiaTunePerugia0(); 
  
 AliGenBox *box = new AliGenBox(1);
 box->SetThetaRange(thmin,thmax); 
 box->SetPhiRange(0., 360.); 

 if (gRandom->Rndm() < 0.1)    // 10% of events with deu.
   {
     box->SetPart(1000010020);   // for deu  
     box->SetMomentumRange(0.2, 3.0);
   } else if ( (gRandom->Rndm() >= 0.1) && (gRandom->Rndm() < 0.2) )
   {
     box->SetPart(-1000010020);   // for anti-deu  
     box->SetMomentumRange(0.2, 3.0);
   } else if ( (gRandom->Rndm() >= 0.2) && (gRandom->Rndm() < 0.3) )
   {
     box->SetPart(1000010030);   // for triton
     box->SetMomentumRange(0.2, 3.0);
   } else if ( (gRandom->Rndm() >= 0.3) && (gRandom->Rndm() < 0.4) )
   {
     box->SetPart(-1000010030);   // for anti-triton
     box->SetMomentumRange(0.2, 3.0);
   } else if ( (gRandom->Rndm() >= 0.4) && (gRandom->Rndm() < 0.55) )
   {
     box->SetPart(1000020030);   // for Helium3
     box->SetMomentumRange(0.3, 8.0);
   } else if ( (gRandom->Rndm() >= 0.55) && (gRandom->Rndm() < 0.7) )
   {
     box->SetPart(-1000020030);   // for anti-Helium3
     box->SetMomentumRange(0.3, 8.0);
   } else if ( (gRandom->Rndm() >= 0.7) && (gRandom->Rndm() < 0.85) )
   {
     box->SetPart(1000020040);   // for Helium4
     box->SetMomentumRange(0.3, 8.0);
   } else if (gRandom->Rndm() >= 0.85)
   {
     box->SetPart(-1000020040);   // for anti-Helium4
     box->SetMomentumRange(0.3, 8.0);
   }

 Cocktail->AddGenerator(pythia,"pythiaPerugia",1);  
 Cocktail->AddGenerator(box,"fbox",1); 
