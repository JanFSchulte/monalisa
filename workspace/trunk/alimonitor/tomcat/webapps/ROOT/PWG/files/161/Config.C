//
// Configuration for MC 2010
// pp collisions at 7 TeV
//
// Reference NLO MNR for c and b: 7 TeV CTEQ6M, mc=1.2,muR=muF=2mu0
//                                              mb=4.75,muR=muF=mu0
//
// sigma(ccbar) = 6.91 mb
// sigma(bbbar) = 0.232 mb
//
// sigma(pp,inel,PYTHIA) = 79 mb
//
// 90.46% of MSEL=0 events (including diffractive) with
// QQbar switched off (these events will include injected J/psi) => kPyMbNoHvq
//
// 8.75% of MSEL=1 events with ccbar (in 4 subsamples) => kCharmpp7000wmi
//     bin 1 45% (3.938%): 3 < pthard < 4 GeV/c
//     bin 2 35% (3.062%): 4 < pthard < 6 GeV/c
//     bin 3 10% (0.875%): 6 < pthard < 8 GeV/c
//     bin 4 10% (0.875%):  pthard > 8 GeV/c
//
// 0.29% of MSEL=1 events with bbbar (in 4 subsamples) => kBeautypp7000wmi
...
//--- Heavy Flavour Production ---
  kCharmPbPb5500,  kCharmpPb8800,  kCharmpp14000,  kCharmpp7000wmi,
  kCHarmpp14000wmi,
  kD0PbPb5500,     kD0pPb8800,     kD0pp14000,
  kDPlusPbPb5500,  kDPluspPb8800,  kDPluspp14000,
  kBeautyPbPb5500, kBeautypPb8800, kBeautypp14000, kBeautypp7000wmi,
  kBeautypp14000wmi,
// -- Pythia Mb
...
//*******************************************************************//
  ...
  case kCharmpp14000:
      ...
      break;
  case kCharmpp7000wmi:
      comment = comment.Append(" Charm in pp at 7 TeV with mult. interactions");
      gener = new AliGenPythia(-1);
      gener->SetProcess(kPyCharmppMNRwmi);
      gener->SetStrucFunc(kCTEQ6l); // start using CTEQ6
  ...
  case kBeautypp14000:
      ...
      gener->SetEnergyCMS(energy);
      break;
  case kBeautypp7000wmi:
      comment = comment.Append(" Beauty in pp at 7 TeV with mult. interactions");
      gener = new AliGenPythia(-1);
...
void ProcessEnvironmentVars()
{
	...
	if (strcmp(gSystem->Getenv("DC_RUN_TYPE"), pprRunName[iRun])==0) {
	  proc = (PDC10Proc_t)iRun;
	  cout<<"Run type set to "<<pprRunName[iRun]<<endl;
	}
      }
    } else {
      // pp 7 wmi with c (D's forced hadronic)
      decHvFl = kHadr;
      proc = kCharmpp7000wmi;

      // Define ptHardMin,ptHardMax
      // The array below contains the cumulative probability
      // for the different pthard bins
      Double_t probPtCharm[] = {0.00,0.45,0.80,0.90,1.00};
	...
	cout << "ProcessEnvironmentVars: Wrong pthard bin" << endl;
      ...
      cout<<"Decay set to kHadronicD"<<endl;
