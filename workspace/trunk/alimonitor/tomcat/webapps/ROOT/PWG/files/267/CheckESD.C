...
TH1F* Create1DHistos(Int_t nMod,TString hname)
{
  // add my satya: pmd adc plots
  char     name[50];
  char     title[100] ;
  Int_t    nBins  = 100;
  Double_t xMin   = 0;
  ...
  sprintf(name,"%s%d",hname.Data(),nMod);
  if(nMod < 24)
    sprintf(title,"Energy Deposition [ncell < 3] in SM%d (PRE)/(ESD)",nMod);
  else
    sprintf(title,"Energy Deposition [ncell < 3] in SM%d (CPV)/(ESD)",nMod);
