<%@ page import="lia.Monitor.Store.Fast.DB,lia.Monitor.Store.Cache,lia.Monitor.Store.FarmBan,lia.web.utils.Formatare,lia.Monitor.monitor.monPredicate,java.io.*,java.util.*,java.net.URLEncoder,lia.web.servlets.web.*,lia.web.utils.*,lia.Monitor.monitor.Result,lia.Monitor.monitor.eResult,lia.Monitor.JiniClient.Store.Main" %><%
    lia.web.servlets.web.Utils.logRequest("START /admin/linux.jsp", 0, request);
    ...
    String sQuery = "SELECT vobox_stats.*,abping_aliases.libc_ver FROM vobox_stats INNER JOIN abping_aliases ON vobox_stats.name=abping_aliases.name ORDER BY lower(vobox_stats.name) ASC, vobox_stats.name";
    ...
    sQuery = "SELECT linuxflavor, count(1) as cnt FROM vobox_stats INNER JOIN abping_aliases ON vobox_stats.name=abping_aliases.name GROUP BY linuxflavor ORDER BY cnt DESC, lower(linuxflavor) ASC, linuxflavor;";
    db.query(sQuery);
    ...
    lia.web.servlets.web.Utils.logRequest("/admin/linux.jsp", s.length(), request);
%>
