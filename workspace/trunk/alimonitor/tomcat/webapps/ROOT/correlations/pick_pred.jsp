<%@ page import="lazyj.Format,java.util.*,java.io.*,java.text.SimpleDateFormat,java.awt.Color,lia.web.utils.*,lia.Monitor.Store.Fast.DB,lia.web.servlets.web.*,lia.web.utils.*,lia.Monitor.monitor.*,lia.Monitor.Store.*"%><%
	...
	if ("*".equals(sFilterFarm))
    	...
    	sFilterFarm = Format.replace(sFilterFarm, "*", "%");
    	sFilterCluster = Format.replace(sFilterCluster, "*", "%");
    	sFilterNode = Format.replace(sFilterNode, "*", "%");
    	sFilterParameter = Format.replace(sFilterParameter, "*", "%");
	...
	String q = "SELECT name FROM (SELECT distinct split_part(mi_key,'/',1) AS name FROM monitor_ids"+sWhereFarm+") AS x ORDER BY lower(name) ASC;";
	...
	db.query("SELECT distinct split_part(mi_key,'/',2) FROM monitor_ids"+sWhereCluster+" ORDER BY 1;");
	while (db.moveNext()){
	    String s = db.gets(1);
	...
	db.query("SELECT distinct split_part(mi_key,'/',3) FROM monitor_ids"+sWhereNode+" ORDER BY 1;");
	while (db.moveNext()){
	    String s = db.gets(1);
	...
	db.query("SELECT distinct split_part(mi_key,'/',4) FROM monitor_ids"+sWhereParameter+" ORDER BY 1;");
	while (db.moveNext()){
	    String s = db.gets(1);
