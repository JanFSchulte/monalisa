...
function read_subwagon(list, highest, variable, suffix)
{
  for(i = 1; i<= highest;i++){
    list[i] = objById(variable+i+suffix).value;
  }
  return list;
}

function read_subwagon_checked(list, highest)
{
  for(i = 1; i<= highest;i++){
    list[i] = objById("subwagon_"+i+"_checkbox").checked;
  }
  return list;
}
...
function add_subwagon()
{
  ...
  var old = wagon_list.innerHTML;
  var highest = parseInt(objById("highest_subwagon").value);
  old_value_list = new Array(highest);
  old_value_name = new Array(highest);
  old_value_checked = new Array(highest);
  old_value_list = read_subwagon(old_value_list, highest, "subwagon_", "");
  old_value_name = read_subwagon(old_value_name, highest, "subwagon_", "_name");
  old_value_checked = read_subwagon_checked(old_value_checked, highest);

  highest = highest + 1;
  var name = "subwagon_"+highest;
  if(highest>1)
     old = old +' <br>'
  wagon_list.innerHTML = old + ' <br>subwagon name <input type=text size=20 name='+name+'_name id='+name+'_name class=input_text> <input type=checkbox name='+name+'_checkbox id='+name+'_checkbox value=1 checked>  subwagon activated <br> <td> <textarea rows=3 cols=68 class=input_textarea name='+name+' id='+name+'></textarea> </td> ';
 ...
 write_subwagon_checked(old_value_checked, highest-1);
  ...
  objById("subwagon_"+highest+"_name").value = highest;
...
function remove_subwagon()
{
  ...
  var subwagon_name = objById("subwagon_"+objById("highest_subwagon").value+"_name").value;
...
<table border=0 cellspacing=0 cellpadding=2 class=text>
   ...
   <<:com_train_operator_start:>>
	...
	<b>Subwagons are<br>activated!</b></td>
	<td>
	    <<:com_edit_start:>>
...
<table border=0 cellspacing=0 cellpadding=2 class=text>
    <tr>
      <td>
	<b>You can run this wagon with multiple differenct configurations. For each subwagon, one wagon is created in the train. The customizations below are individually attached to the macro customization. <font color="red">Important</font>: Your AddTask macro must support this, i.e. the last parameter of your AddTask macro must be a suffix to the output container name. Please click <a href="https://twiki.cern.ch/twiki/bin/viewauth/ALICE/AnalysisTrains#subwagons">here</a> for more information.</b>
      </td>
    </tr>
	...
	<td>
	  ...
	  <input type=submit name=submit value="Remove subwagon" class=input_submit onClick="return remove_subwagon()">
	  <br> 
	</td>
	<<:com_edit_end:>>
    </tr>
