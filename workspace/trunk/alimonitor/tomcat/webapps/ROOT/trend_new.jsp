<%@ page import="alimonitor.*,lazyj.RequestWrapper,lia.web.utils.ServletExtension,lia.web.utils.Formatare,java.util.*,java.io.*,java.text.SimpleDateFormat,java.awt.Color,lia.Monitor.Store.Fast.DB,lia.web.servlets.web.*,lia.Monitor.monitor.*,lia.Monitor.Store.*"%><%
    lia.web.servlets.web.Utils.logRequest("START /trend_new.jsp", 0, request);

    response.setHeader("Connection", "keep-alive");

    RequestWrapper rw = new RequestWrapper(request);

    int iSort = rw.geti("sort", 1);
    int iSortType = rw.geti("type", 0);
    int iFilter = rw.geti("filter", 0);
    ...
    pMaster.modify("bookmark", "/trend_new.jsp");
    ...
    String sQuery = "select '_TOTALS_' as name,(select sum(cpus) from pledged_dynamic) as cpus,(select sum(ksi2k) from pledged_dynamic) as ksi2k UNION select * FROM (SELECT name,cpus,ksi2k from abping_aliases left outer join pledged_dynamic on name=site where (select count(1) from monitor_ids where mi_key=name||'/Site_Jobs_Summary/jobs/RUNNING_jobs')=1 ";
	...
	final int iksi2k = hmSI2K.containsKey(sSite) ? hmSI2K.get(sSite) : 0;
    ...
    lia.web.servlets.web.Utils.logRequest("/trend_new.jsp", baos.size(), request);
%>